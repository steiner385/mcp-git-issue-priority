  ...
  server.tool(
    ...
    async (args) => {
      ...
      if (!parsed && args.issueNumber) {
        return {
          content: [{ type: 'text', text: JSON.stringify({ success: false, error: "Repository required when specifying issue number. Provide 'repository' argument or set GITHUB_REPOSITORY env var.", code: 'REPO_REQUIRED' }) }],
          isError: true,
        };
      }
